    ...
    pub fn set_result(&mut self, result: QueryResult, cx: &mut Context<Self>) {
        self.column_widths = result
            ...
            .map(|col| -> Pixels {
                ...
                let header_width = px(col.name.len() as f32 * 8.0 + 16.0);
...
impl Render for ResultsGrid {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .bg(bg)
            // Header row
            .child(self.render_header_row(surface, border_color, text_color, muted))
            // Data rows
            .child(
                div().flex_1().child(
                    uniform_list(
                        ...
                        cx.processor(move |this: &mut Self, range: std::ops::Range<usize>, _window: &mut Window, cx: &mut Context<Self>| {
                            ...
                            range
                                .map(|ix| this.render_data_row(ix, bg, surface, border_color, text_color, muted, number_color))
                                .collect()
                        }),
                    )
                    .h_full()
